...
interface ISeaweedStakingERC20 is ISeaweedStaking {
    function sendProfit(uint amount) external;
}

interface ISeaweedOptions {
    event Create(
        uint256 indexed id,
...
/**
 * @title Seaweed HBTC Liquidity Pool
 * @notice Accumulates liquidity in HBTC from LPs and distributes P&L in HBTC
 */
contract SeaweedHBTCPool is IERCLiquidityPool, Ownable, ERC20("Seaweed HBTC LP Token", "writeHBTC")
{
    using SafeMath for uint256;
    ...
    /* 
     * @nonce calls by SeaweedPutOptions to lock funds
    ...
    function lock(uint id, uint256 amount, uint256 premium) external override onlyOwner {
        ...
        require(lockedAmount.add(amount).mul(10) <= totalBalance().mul(8), "Pool Error: Amount is too large.");
    ...
    /* 
     * @nonce Calls by SeaweedPutOptions to unlock funds
    ...
    /* 
     * @nonce calls by SeaweedPutOptions to unlock the premiums after an option's expiraton
     * @param to Provider
     * @param amount Amount of premiums that should be unlocked
     */
    /* 
     * @nonce calls by SeaweedCallOptions to send funds to liquidity providers after an option's expiration
     * @param to Provider
     * @param amount Funds that should be sent
     */
    function send(uint id, address payable to, uint256 amount) external override onlyOwner {
        LockedLiquidity storage ll = lockedLiquidity[id];
        require(ll.locked, "LockedLiquidity with such id has already unlocked");
    ...
    /* 
     * 
     * @nonce A provider supplies HBTC to the pool and receives writeHBTC tokens
    ...
    /* 
     * @nonce Provider burns writeHBTC and receives HBTC from the pool
     * @param amount Amount of HBTC to receive
    ...
    /* 
     * @nonce Returns provider's share in HBTC
     * @param account Provider's address
     * @return Provider's share in HBTC
     */
    ...
    /* 
     * @nonce Returns the amount of HBTC available for withdrawals
    ...
    /* 
     * @nonce Returns the HBTC total balance provided to the pool
    ...
    function _beforeTokenTransfer(address from, address to, uint256) internal override {
        if (lastProvideTimestamp[from].add(lockupPeriod) > block.timestamp && lastProvideTimestamp[from] > lastProvideTimestamp[to]) {
...
/** 
 * @title Seaweed HBTC (Wrapped Bitcoin) Bidirectional (Call and Put) Options
 * @notice Seaweed Protocol Options Contract
 */
contract SeaweedHBTCOptions is Ownable, ISeaweedOptions {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    ISeaweedStakingERC20 public settlementFeeRecipient;
    ...
    SeaweedHBTCPool public pool;
    
    IMdexRouter public mdexRouter;
    
    address[] public htToHBTCSwapPath;
    
    IERC20 public HBTC;

    /** 
     * @param _priceProvider The address of Oracle BTC/USD price feed contract
     * @param __mdexRouter The address of _mdexRouter router contract
     * @param token The address of HBTC ERC20 token contract
     */
    constructor(AggregatorV3Interface _priceProvider, IMdexRouter __mdexRouter, ERC20 token, ISeaweedStakingERC20 _settlementFeeRecipient) public {
        pool = new SeaweedHBTCPool(token);
        HBTC = token;
        priceProvider = _priceProvider;
        settlementFeeRecipient = _settlementFeeRecipient;
        impliedVolRate = 5500;
        mdexRouter = __mdexRouter;
        contractCreationTimestamp = block.timestamp;
        
        approve();

        htToHBTCSwapPath = new address[](2);
        htToHBTCSwapPath[0] = mdexRouter.WHT();
        htToHBTCSwapPath[1] = address(HBTC);
    }
     ...
     */
    function setSettlementFeeRecipient(ISeaweedStakingERC20 recipient) external onlyOwner {
        require(block.timestamp < contractCreationTimestamp + 14 days);
     ...
     * @param strike Strike price of the option 行权价
     * @param optionType Call or Put option type
     * @return optionID Created option's ID
     */
    function create(uint256 period, uint256 amount, uint256 strike, OptionType optionType) external payable returns (uint256 optionID)
    {
        (uint256 total, uint256 totalHT, uint256 settlementFee, uint256 strikeFee, ) = fees(period, amount, strike, optionType);
        require(optionType == OptionType.Call || optionType == OptionType.Put, "Wrong option type" );
        ...
        uint amountIn = swapToHBTC(totalHT, total);
    ...
    function approve() public {
        HBTC.safeApprove(address(pool), uint(-1));
        HBTC.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
     ...
     * @return totalHT Total price in ETH to be paid
     * @return  settlementFee Amount to be distributed to the Seaweed token holders
     * @return  strikeFee Amount that covers the price difference in the ITM options 
     * @return  periodFee Option period fee amount 
     */
    function fees(uint256 period, uint256 amount, uint256 strike, OptionType optionType) public view returns (uint256 total, uint256 totalHT, uint256 settlementFee, uint256 strikeFee, uint256 periodFee)
    {
        ...
        totalHT = mdexRouter.getAmountsIn(total, htToHBTCSwapPath)[0];
    }
     ...
     */
    function getSettlementFee(uint256 amount) internal pure returns (uint256 fee)
    {
        return amount / 100 * 2;
    }
     ...
     */
    function getPeriodFee(uint256 amount, uint256 period, uint256 strike, uint256 currentPrice, OptionType optionType) internal view returns (uint256 fee) {
        if (optionType == OptionType.Put)
            return amount.mul(sqrt(period)).mul(impliedVolRate).mul(strike).div(currentPrice).div(PRICE_DECIMALS);
        else
            return amount.mul(sqrt(period)).mul(impliedVolRate).mul(currentPrice).div(strike).div(PRICE_DECIMALS);
    }
     ...
     */
    function getStrikeFee(uint256 amount, uint256 strike, uint256 currentPrice, OptionType optionType ) internal pure returns (uint256 fee) {
    ...
    /** 
     * @notice Sends profits in HBTC from the HBTC pool to an option holder's address
    ...
    /**
     * @notice Swap HT to HBTC via _mdexRouter router
     * @param maxAmountIn The maximum amount of HT that can be required before the transaction reverts.
     * @param amountOut The amount of HBTC tokens to receive.
     */
    function swapToHBTC(uint maxAmountIn, uint amountOut) internal returns (uint)
    {
            // swapETHForExactTokens alias swapHTForExactTokens
            uint[] memory amounts = mdexRouter.swapETHForExactTokens {
                value: maxAmountIn
            }(
                amountOut,
                htToHBTCSwapPath,
