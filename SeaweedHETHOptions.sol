...
/**
 * @title Seaweed HETH Liquidity Pool
 * @notice Accumulates liquidity in HETH from LPs and distributes P&L in HETH
 */
contract SeaweedHETHPool is IERCLiquidityPool, Ownable, ERC20("Seaweed HETH LP Token", "writeHETH")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies HETH to the pool and receives writeHETH tokens
    ...
    /*
     * @nonce Provider burns writeHETH and receives HETH from the pool
     * @param amount Amount of HETH to receive
    ...
    /*
     * @nonce Returns provider's share in HETH
     * @param account Provider's address
     * @return Provider's share in HETH
     */
    ...
    /*
     * @nonce Returns the amount of HETH available for withdrawals
    ...
    /*
     * @nonce Returns the HETH total balance provided to the pool
...
/**
 * @title Seaweed HETH (Wrapped Bitcoin) Bidirectional (Call and Put) Options
 * @notice Seaweed Protocol Options Contract
 */
contract SeaweedHETHOptions is Ownable, ISeaweedOptions {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    SeaweedHETHPool public pool;
    IMdexRouter public mdexRouter;
    address[] public ethToHETHSwapPath;
    IERC20 public HETH;
     ...
     * @param token The address of HETH ERC20 token contract
     */
    constructor(AggregatorV3Interface _priceProvider, IMdexRouter __mdexRouter, ERC20 token, ISeaweedStakingERC20 _settlementFeeRecipient) public {
        pool = new SeaweedHETHPool(token);
        HETH = token;
        ...
        ethToHETHSwapPath = new address[](2);
        ethToHETHSwapPath[0] = mdexRouter.WHT();
        ethToHETHSwapPath[1] = address(HETH);
    ...
    {
        (uint256 total, uint256 totalHT, uint256 settlementFee, uint256 strikeFee, ) =
        ...
        uint amountIn = swapToHETH(totalHT, total);
    ...
    function approve() public {
        HETH.safeApprove(address(pool), uint(-1));
        HETH.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalHT = mdexRouter.getAmountsIn(total, ethToHETHSwapPath)[0];
    }
    ...
    /**
     * @notice Sends profits in HETH from the HETH pool to an option holder's address
    ...
    /**
     * @notice Swap HT to HETH via _mdexRouter router
     * @param maxAmountIn The maximum amount of HT that can be required before the transaction reverts.
     * @param amountOut The amount of HETH tokens to receive.
     */
    function swapToHETH(uint maxAmountIn, uint amountOut) internal returns (uint)
    {
