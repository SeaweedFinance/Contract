...
interface ISeaweedStakingHT is ISeaweedStaking {
...
interface IHTLiquidityPool is ILiquidityPool {
...
/**
 * @title Seaweed HT Liquidity Pool
 * @notice Accumulates liquidity in HT from LPs and distributes P&L in HT
 */
contract SeaweedHTPool is IHTLiquidityPool, Ownable, ERC20("Seaweed HT LP Token", "writeHT")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies HT to the pool and receives writeHT tokens
    ...
    /*
     * @nonce Provider burns writeHT and receives HT from the pool
     * @param amount Amount of HT to receive
    ...
    /*
     * @nonce calls by SeaweedCallOptions to lock the funds
    ...
    /*
     * @nonce calls by SeaweedOptions to unlock the funds
    ...
    /*
     * @nonce Returns provider's share in HT
     * @param account Provider's address
     * @return Provider's share in HT
     */
    ...
    /*
     * @nonce Returns the amount of HT available for withdrawals
    ...
    /*
     * @nonce Returns the total balance of HT provided to the pool
...
/**
 * @title Seaweed HT (HT) Bidirectional (Call and Put) Options
 * @notice Seaweed HT Options Contract
 */
contract SeaweedHTOptions is Ownable, ISeaweedOptions {
    using SafeMath for uint256;

    ISeaweedStakingHT public settlementFeeRecipient;
    ...
    SeaweedHTOptions private oldSeaweedHTOptions;
    AggregatorV3Interface public priceProvider;
    SeaweedHTPool public pool;

    /**
     * @param pp The address of HBO HT/USD price feed contract
     */
    constructor(AggregatorV3Interface pp, ISeaweedStakingHT staking, SeaweedHTPool _pool) public {
        pool = _pool;
        priceProvider = pp;
     ...
     */
    function setSettlementFeeRecipient(ISeaweedStakingHT recipient) external onlyOwner {
        require(block.timestamp < contractCreationTimestamp + 14 days);
    ...
    function migrate(uint count) external onlyOwner {
        ...
        require(address(oldSeaweedHTOptions) != address(0));
        for (uint i = 0; i < count; i++){
            uint optionID = options.length;
            SeaweedHTOptions.Option memory option;
            (
                option.state,
                option.holder,
                option.strike,
                ...
                option.expiration,
                option.optionType
            ) = oldSeaweedHTOptions.options(optionID);
    ...
    function setOldSeaweedHTOptions(address oldAddr) external onlyOwner {
        require(address(oldSeaweedHTOptions) == address(0));
        oldSeaweedHTOptions = SeaweedHTOptions(oldAddr);
    }
     ...
     */
    function fees(uint256 period, uint256 amount, uint256 strike, OptionType optionType) public view returns (uint256 total, uint256 settlementFee, uint256 strikeFee, uint256 periodFee)
    {
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of HT
    ...
    /**
     * @notice Sends profits in HT from the HT pool to an option holder's address
