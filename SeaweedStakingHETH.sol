...
abstract contract SeaweedStaking is ERC20, ISeaweedStaking {
    using SafeERC20 for IERC20;
    using SafeMath for uint;

    IERC20 public immutable Seaweed;

    uint public constant MAX_SUPPLY = 500;

    uint public constant POT_PRICE = 200_000e18;
    ...
    constructor(ERC20 _token, string memory name, string memory short) public ERC20(name, short)
    {
        Seaweed = _token;
        _setupDecimals(0);
    ...
    function buy(uint amount) external override {
        ...
        Seaweed.safeTransferFrom(msg.sender, address(this), amount.mul(POT_PRICE));
    }

    function sell(uint amount) external override lockupFree {
        _burn(msg.sender, amount);
        Seaweed.safeTransfer(msg.sender, amount.mul(POT_PRICE));
    }
...
contract SeaweedStakingHETH is SeaweedStaking, ISeaweedStakingERC20 {
    using SafeERC20 for IERC20;
    using SafeMath for uint;

    IERC20 public immutable HETH;

    constructor(ERC20 _token, ERC20 _HETH) public SeaweedStaking(_token, "Seaweed HETH Staking POT", "slHETH") {
        HETH = _HETH;
    }
    ...
    function sendProfit(uint amount) external override {
        ...
        if (_totalSupply > 0) {
            ...
            HETH.safeTransferFrom(msg.sender, FALLBACK_RECIPIENT, amount);
        }
    }
